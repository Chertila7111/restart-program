...
export async function GET() {
  ...
  try {
    ...
    const rows = await (prisma as any).$queryRawUnsafe(
      `SELECT speciality, approach, education, experience, bio, workStyle, quote, photoUrl FROM "PsychologistProfile" WHERE userId = ? LIMIT 1`,
      userId
    )
    ...
    if (exists && exists.length > 0) {
      await (prisma as any).$executeRawUnsafe(
        `UPDATE "PsychologistProfile"
         SET speciality = ?, approach = ?, education = ?, experience = ?, bio = ?, workStyle = ?, quote = ?, photoUrl = ?, updatedAt = CURRENT_TIMESTAMP
         WHERE userId = ?`,
        speciality, approach, education, experience, bio, workStyle, quote, photoUrl, userId
      )
    } else {
      const id = `psych-${Date.now()}`
      await (prisma as any).$executeRawUnsafe(
        `INSERT INTO "PsychologistProfile" (id, userId, speciality, approach, education, experience, bio, workStyle, quote, photoUrl, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
        id, userId, speciality, approach, education, experience, bio, workStyle, quote, photoUrl
      )
    }
