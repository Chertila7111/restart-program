...
export const metadata: Metadata = {
  title: 'Снова с собой — поддержка в сложных отношениях и после расставания',
  description:
    'Бережная 4-недельная программа с психологом и небольшой группой. Для тех, кто устал от конфликтов, переживает расставание или хочет вернуть опору на себя — постепенно, без давления.',
}

const scenarios = [
  { id: 'thoughts', emoji: '💭', title: 'Мысли не останавливаются', desc: 'Постоянно прокручиваю разговоры, не могу остановить голову' },
  { id: 'routine', emoji: '🌀', title: 'Выпал(а) из режима', desc: 'Работа, сон, питание — всё разладилось' },
  { id: 'anxiety', emoji: '🌫️', title: 'Тревога или пустота', desc: 'Не понимаю что со мной — странное состояние внутри' },
  { id: 'conflicts', emoji: '⚡', title: 'Устал(а) от конфликтов', desc: 'Ссоры по кругу, напряжение, не знаю что делать' },
  { id: 'lost', emoji: '💙', title: 'Теряю себя', desc: 'Рядом с партнёром или после расставания — чувство потери себя' },
  { id: 'future', emoji: '🧭', title: 'Не понимаю, что дальше', desc: 'Жизнь как будто остановилась, непонятно куда двигаться' },
]
...
const layers = [
  {
    num: '1',
    step: 'Сначала',
    title: 'Разобраться, что происходит',
    desc: 'Психолог помогает назвать состояние: боль, тревога, усталость от конфликтов, желание уйти или вернуть. Безопасно, без осуждения и без давления.',
    tag: 'Ясность',
  },
  {
    num: '2',
    step: 'Потом',
    title: 'Вернуть опору на себя',
    desc: 'Группа, чат с куратором, задания, дневник — понятная структура каждой недели. Перестаёшь чувствовать себя одиноким в этом.',
    tag: 'Поддержка',
  },
  {
    num: '3',
    ...
    desc: 'Режим, самооценка, границы, цели. Когда состояние становится устойчивее — появляется ясность, что делать дальше.',
    tag: 'Движение',
  },
]
...
const weeks = [
  ...
  {
    num: 'Неделя 2', title: 'Границы и честность с собой',
    onMeeting: 'Разбираем, что держит: чувство вины, привязанность, страх, надежда. Работа с импульсами',
    afterMeeting: 'Практика паузы перед импульсивными действиями — три шага в острый момент',
...
const faqs = [
  { q: 'Подходит ли программа, если я ещё в отношениях?', a: 'Да. Программа помогает разобраться с собой — независимо от того, вместе вы сейчас или нет. Если вы устали от конфликтов, не понимаете, что чувствуете, или теряете себя рядом с партнёром — это тоже повод прийти. На встрече не будут подталкивать к расставанию.' },
  { q: 'Это подходит для мужчин?', a: 'Да. Программа создана для всех — независимо от пола. Есть и отдельные группы для мужчин, где разговор идёт в более комфортном формате.' },
  ...
  { q: 'Как быстро будет результат?', a: 'Многие участники замечают первые сдвиги уже после первой встречи. К концу программы обычно появляется больше структуры, понимания своего состояния и план дальнейших шагов. Результат зависит от ситуации и вовлечённости.' },
  { q: 'Нужно ли принимать решение расставаться — до прихода на встречу?', a: 'Нет. На встрече нет задачи подтолкнуть к расставанию или к тому, чтобы остаться. Можно прийти именно тогда, когда ещё непонятно, что делать. Это и есть хорошая точка входа.' },
  ...
  { q: 'Есть ли возврат средств?', a: 'Возврат возможен за неоказанные услуги. Вводную можно отменить до начала. По программе и пакетам — возврат за оставшиеся неоказанные встречи. Уже проведённые встречи не возвращаются. Если встреча не состоялась по нашей вине — предложим новую дату или вернём стоимость этой встречи.' },
  ...
  { q: 'Что если я пойму, что группа мне не подходит?', a: 'Мы честно скажем об этом ещё на вводной встрече, если заметим, что формат не ваш. При отказе от программы возвращается стоимость оставшихся неоказанных встреч. Вводная встреча — разовая услуга, возврат возможен только до её начала.' },
...
const faqSchemaItems = [
  { q: 'Что такое программа «Снова с собой»?', a: '«Снова с собой» — 4-недельная онлайн-программа с психологом и небольшой группой. Подходит как для тех, кто переживает расставание или развод, так и для тех, кто устал от конфликтов в отношениях и хочет разобраться с собой.' },
  ...
  { q: 'Подходит ли программа, если я ещё в отношениях?', a: 'Да. Программа помогает разобраться с собой независимо от ситуации — в конфликтных отношениях, при эмоциональной усталости или после расставания.' },
  { q: 'Как пережить расставание или трудный период в отношениях?', a: 'Программа «Снова с собой» помогает через стабилизацию состояния, работу с границами и самооценкой, восстановление режима и построение личного плана.' },
]
          ...
          <div className="grid-hero" style={{
            ...
            <div>
              ...
              <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', lineHeight: 1.7, maxWidth: '32rem' }}>
                Тест бесплатный — поможет понять, какой формат поддержки подойдёт именно вам.
              </p>
            </div>
            ...
            <div className="hide-mobile" style={{ display: 'flex', justifyContent: 'center' }}>
              <div style={{
                ...
                {/* Price CTA */}
                <Link href="/checkout?product=intro" style={{
          ...
          <div className="grid-two" style={{ display: 'grid', gap: '2rem', alignItems: 'start' }}>
            ...
            <div>
              ...
              <div style={{ background: 'white', borderRadius: '1.5rem', padding: '2.5rem', marginBottom: '1.5rem', border: '1.5px solid var(--border)', position: 'relative' }}>
                <div style={{ fontSize: '5rem', lineHeight: 0.8, color: 'var(--primary)', opacity: 0.12, fontFamily: 'Georgia, serif', position: 'absolute', top: '1.5rem', left: '1.75rem', userSelect: 'none' }}>&ldquo;</div>
      ...
      <section className="section" style={{ background: 'var(--bg-sage)' }}>
        <div className="container mx-auto px-6" style={{ textAlign: 'center', maxWidth: '44rem' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '1.05rem', lineHeight: 1.8, marginBottom: '2.5rem' }}>
            Вступайте в следующую группу. Уже на вводной можно чуть лучше понять, что с вами происходит — и есть ли смысл идти дальше.
          </p>
